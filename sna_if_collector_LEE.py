...
import SNA_config_equip as cfg_e
import threading
import time
...
class SnaIfTrfCollector(object):
    ...
    def make_equip_data(self, if_datas):
    #equipment 테이블 수집
        trf_dict = {}
        for key, cur_data in if_datas.items():
            try:
                ...
                try:
                    ...
                    print(cur_data['TRF_TYPE'])
                    print(" = ")
                    print(cfg_e.EQUIP_COLLECT_OIDS_DICT['sysName'])
                    if (cur_data['TRF_TYPE'] == cfg_e.EQUIP_COLLECT_OIDS_DICT['sysName']) :
                        tmp_dict['E_NAME'] = cur_data['VALUE']

                    if (cur_data['TRF_TYPE'] == cfg_e.EQUIP_COLLECT_OIDS_DICT['ipAdEntAddr']) :
                        tmp_dict['E_IP'] = cur_data['VALUE']

                    if (cur_data['TRF_TYPE'] == cfg_e.EQUIP_COLLECT_OIDS_DICT['sysDescr']) :
                        tmp_dict['E_DESCR'] = cur_data['VALUE']

                    if (cur_data['TRF_TYPE'] == cfg_e.EQUIP_COLLECT_OIDS_DICT['sysLocation']) :
                        tmp_dict['E_LOCATION'] = int(cur_data['VALUE']) * cfg_e.HIGH_SPEED
                ...
                print('Exception occur - ', e)
                print(" tmp_dict['E_IP'] = ",  tmp_dict['E_IP'])
                print(" tmp_dict['E_NAME'] = ",  tmp_dict['E_NAME'])
                print(" tmp_dict['E_DESCR'] = ",  tmp_dict['E_DESCR'])
                print(" tmp_dict['E_LOCATION'] = ",  tmp_dict['E_LOCATION'])
    ...
    for target in targets:
        try:
            if_data = if_collector.get_snmp_traffic(target[0],
                    ...
                    cfg.SNMP_COUNT_OID,
                    cfg_e.EQUIP_COLLECT_OIDS_DICT.values(),
                    cfg_e.EQUIP_COLLECT_OIDS
            )
            # print("if_data = ")
            # print(if_data)
            # print
            ...
            save_if = if_collector.make_equip_data(save_if_data)
            print("save_if = ",save_if)
            print('\n')
            save_count = mysql_exec.save_equip_list(save_if)
            ...
            if(save_count == False) :
                ...
                print("no data")
                mysql_exec.save_equip_list(save_if)
            mysql_exec.commit()
        except Exception as e:
            print(e)
            continue
